...
def run_hyperparameter_search():
    ...
    feature_signature = get_feature_signature(features.columns)
    ...
    for model_name, (model, search_space) in searches.items():
        ...
        with tqdm(total=HYPERPARAMETER_SEARCH_ITERATIONS, desc=model_name) as progress_bar:

            def on_step(result):
                progress_bar.update(1)
                progress_bar.set_postfix(best_score=f"{-result.fun:.4f}")
                return False
        ...
        previous_score = (
            ...
            if hyperparameter_result_matches(previous_result, features.columns, len(features))
            else float("-inf")
        )
